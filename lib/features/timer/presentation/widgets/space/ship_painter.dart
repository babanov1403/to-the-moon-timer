...
void drawShip(Canvas canvas, Offset pos, double angle) {
  ...
  const pixels = [
    (4, 0, SpacePalette.shipAccent), (3, 0, SpacePalette.shipBody),
    (2, -1, SpacePalette.shipBody), (1, -1, SpacePalette.shipAccent),
    (0, -1, SpacePalette.shipBody), (-1, -2, SpacePalette.shipBody),
    (-2, -2, SpacePalette.shipDark),
    (2, 1, SpacePalette.shipBody), (1, 1, SpacePalette.shipAccent),
    (0, 1, SpacePalette.shipBody), (-1, 2, SpacePalette.shipBody),
    (-2, 2, SpacePalette.shipDark),
    (2, 0, SpacePalette.shipBody), (1, 0, SpacePalette.shipBody),
    (0, 0, SpacePalette.shipAccent), (-1, 0, SpacePalette.shipBody),
    (-2, 0, SpacePalette.shipDark), (-3, 0, SpacePalette.shipDark),
  ];
...
void drawExhaust(Canvas canvas, Offset pos, double angle, double phase,
  ...
  for (int i = 0; i < count; i++) {
    ...
    final c = Color.lerp(SpacePalette.exhaustWhite, SpacePalette.exhaustOrange, t)!
        .withValues(alpha: (1.0 - t * 0.7) * intensity);
    p.color = c;
    final sz = px * (1.0 - t * 0.5);
    canvas.drawRect(Rect.fromCenter(center: Offset(ex, ey), width: sz, height: sz), p);
  }
  canvas.restore();
}
